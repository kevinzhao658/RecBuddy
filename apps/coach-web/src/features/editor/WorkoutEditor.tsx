...
export function WorkoutEditor({ date: _date, workout, onSave, onClear }: {
  ...
  return (
    <div className="rb-surface flex w-80 flex-col gap-3 border-l border-line p-4">
      <div className="flex flex-wrap gap-1">
        {TYPES.map((t) => <button key={t} onClick={() => set({ type: t })}
          className={`rounded-[20px] px-2.5 py-1 text-xs transition ${d.type === t ? 'bg-accent text-on-accent' : 'bg-chip text-text-mute hover:text-text'}`}>{t}</button>)}
      </div>
      <label className="text-[11px] font-bold uppercase tracking-[0.08em] text-text-mute">Title
        <input aria-label="Title" value={d.title} onChange={(e) => set({ title: e.target.value })}
          className="mt-1 w-full rounded-[10px] border border-line bg-surface2 px-3 py-2 text-[15px] font-normal normal-case tracking-normal text-text" /></label>
      <div className="flex gap-2">
        <label className="flex-1 text-[11px] font-bold uppercase tracking-[0.08em] text-text-mute">Distance (mi)
          <input aria-label="Distance" type="number" value={d.dist ?? ''} onChange={(e) => set({ dist: e.target.value ? Number(e.target.value) : null })}
            className="mt-1 w-full rounded-[10px] border border-line bg-surface2 px-3 py-2 font-num text-[15px] font-normal normal-case tracking-normal text-text" /></label>
        <label className="flex-1 text-[11px] font-bold uppercase tracking-[0.08em] text-text-mute">Pace
          <input aria-label="Pace" value={d.pace ?? ''} onChange={(e) => set({ pace: e.target.value || null })}
            className="mt-1 w-full rounded-[10px] border border-line bg-surface2 px-3 py-2 font-num text-[15px] font-normal normal-case tracking-normal text-text" /></label>
      </div>
      <label className="text-[11px] font-bold uppercase tracking-[0.08em] text-text-mute">Est. time (min) — auto {autoEst}
      ...
      <div className="flex flex-col gap-2">
        ...
        {d.sets.map((p, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <button aria-label={`Remove phase ${i + 1}`} onClick={() => set({ sets: d.sets.filter((_, j) => j !== i) })} className="text-text-faint hover:text-text">✕</button>
          </div>
        ))}
      </div>

      <label className="text-[11px] font-bold uppercase tracking-[0.08em] text-text-mute">Coach's note
        <textarea aria-label="Note" value={d.note} onChange={(e) => set({ note: e.target.value })}
          className="mt-1 w-full rounded-[10px] border border-line bg-surface2 px-3 py-2 text-[15px] font-normal normal-case tracking-normal text-text" rows={3} /></label>
